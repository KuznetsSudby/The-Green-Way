...
open class RangeFloatSeekBarView : View {
    ...
    private fun parseAttr(attrs: AttributeSet?) {
        val typedArray = context.theme.obtainStyledAttributes(attrs, R.styleable.RangeFloatSeekBarView, 0, 0)
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2minValueStep)) {
            minStep = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2minValueStep, 0)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2maxValueStep)) {
            maxStep = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2maxValueStep, 0)
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2minValue)) {
            minValue = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2minValue, 0)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2maxValue)) {
            maxValue = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2maxValue, 0)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2barHeight)) {
            barHeight = typedArray.getDimensionPixelSize(R.styleable.RangeFloatSeekBarView_rf2barHeight, 0)
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2circleRadius)) {
            circleRadius = typedArray.getDimensionPixelSize(R.styleable.RangeFloatSeekBarView_rf2circleRadius, 0)
            defaultPadding = circleRadius
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2circleTextSize)) {
            circleTextSize = typedArray.getDimensionPixelSize(R.styleable.RangeFloatSeekBarView_rf2circleTextSize, 0)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2circleTextColor)) {
            circleTextColor = typedArray.getColor(R.styleable.RangeFloatSeekBarView_rf2circleTextColor, DEFAULT_TEXT_COLOR)
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2circleFillColor)) {
            circleFillColor = typedArray.getColor(R.styleable.RangeFloatSeekBarView_rf2circleFillColor, DEFAULT_CIRCLE_COLOR)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2baseColor)) {
            baseColor = typedArray.getColor(R.styleable.RangeFloatSeekBarView_rf2baseColor, DEFAULT_BASE_COLOR)
        }
        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2fillColor)) {
            fillColor = typedArray.getColor(R.styleable.RangeFloatSeekBarView_rf2fillColor, DEFAULT_FILL_COLOR)
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2CurrentMaxValue)) {
            currentMaxValue = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2CurrentMaxValue, 100)
        }

        if (typedArray.hasValue(R.styleable.RangeFloatSeekBarView_rf2CurrentMinValue)) {
            currentMinValue = typedArray.getInt(R.styleable.RangeFloatSeekBarView_rf2CurrentMinValue, 0)
        }
    ...
    private fun drawDoubleValueBarHorizontal(canvas: Canvas) {
        ...
        val boundsMin = Rect()
        val minValueString = convert(minValueToDraw, count).toString()
        minValuePaint.getTextBounds(minValueString, 0, minValueString.length, boundsMin)

        val boundsMax = Rect()
        val maxValueString = convert(maxValueToDraw, count).toString()
    ...
    private fun convert(number: Int, count: Int): Double {
        return number.toBigDecimal().divide(10.toBigDecimal().pow(count)).setScale(count, RoundingMode.UP).toDouble()
    }
        ...
        when (action and ACTION_MASK) {
            ...
            ACTION_MOVE -> {
                if (pressedThumb != null) {
                    ...
                    mOnDoubleValueSeekBarViewChangeListener?.onValueChanged(this, convert(currentMinValue, count), convert(currentMaxValue, count), true)
            ...
            ACTION_UP -> {
                if (isDragging) {
                    ...
                    mOnDoubleValueSeekBarViewChangeListener?.onStopTrackingTouch(this, convert(currentMinValue, count), convert(currentMaxValue, count))
                } else {
...
interface OnDoubleValueSeekBarChangeListener {
    fun onValueChanged(seekBar: RangeFloatSeekBarView?, min: Double, max: Double, fromUser: Boolean)
    fun onStartTrackingTouch(seekBar: RangeFloatSeekBarView?, min: Double, max: Double)
    fun onStopTrackingTouch(seekBar: RangeFloatSeekBarView?, min: Double, max: Double)
}